...
if (jApp::config()->compilation['checkCacheFiletime']&&(
 ...
 filemtime('C:\webserver\www\lizmap36\lizmap\vendor\jelix\jelix\lib\jelix-modules\jacl2db\daos/jacl2subject.dao.xml') > 1696258090)){ return false;
}
else {
 require_once ( JELIX_LIB_PATH .'dao/jDaoRecordBase.class.php');
 require_once ( JELIX_LIB_PATH .'dao/jDaoFactoryBase.class.php');

class cDaoRecord_jacl2db_Jx_jacl2subject_Jx_sqlite extends jDaoRecordBase {
 public $id_aclsbj;
 public $label_key;
 public $id_aclsbjgrp;
 public $label_group_key;
   public function getSelector() { return "jacl2db~jacl2subject"; }
   public function getProperties() { return cDao_jacl2db_Jx_jacl2subject_Jx_sqlite::$_properties; }
   public function getPrimaryKeyNames() { return cDao_jacl2db_Jx_jacl2subject_Jx_sqlite::$_pkFields; }
}

class cDao_jacl2db_Jx_jacl2subject_Jx_sqlite extends jDaoFactoryBase {
  ...
  array (
   ...
   protected $_DaoRecordClassName='cDaoRecord_jacl2db_Jx_jacl2subject_Jx_sqlite';
...
$__query .=' WHERE  1=1  ORDER BY sbj.id_aclsbjgrp asc, sbj.id_aclsbj asc';
 ...
 function removeRightsFromRightsGroup ($id_aclsbjgrp){
...
$__query .=' WHERE  id_aclsbjgrp '.($id_aclsbjgrp === null ? 'IS NULL' : ' = '.$this->_conn->quote2($id_aclsbjgrp,false)).'';
 ...
 function removeRolesFromGroup ($id_aclsbjgrp){
    $__query = 'UPDATE '.$this->_conn->prefixTable('jacl2_subject').' SET 
 id_aclsbjgrp= NULL';
$__query .=' WHERE  id_aclsbjgrp '.($id_aclsbjgrp === null ? 'IS NULL' : ' = '.$this->_conn->quote2($id_aclsbjgrp,false)).'';
    return $this->_conn->exec ($__query);
}
 function replaceRightsGroup ($old_id_aclsbjgrp, $new_id_aclsbjgrp){
    $__query = 'UPDATE '.$this->_conn->prefixTable('jacl2_subject').' SET 
...
$__query .=' WHERE  id_aclsbjgrp '.($old_id_aclsbjgrp === null ? 'IS NULL' : ' = '.$this->_conn->quote2($old_id_aclsbjgrp,false)).'';
 ...
 function replaceRoleGroup ($old_id_aclsbjgrp, $new_id_aclsbjgrp){
    $__query = 'UPDATE '.$this->_conn->prefixTable('jacl2_subject').' SET 
 id_aclsbjgrp= '.($new_id_aclsbjgrp === null ? 'NULL' : $this->_conn->quote2($new_id_aclsbjgrp,false)).'';
$__query .=' WHERE  id_aclsbjgrp '.($old_id_aclsbjgrp === null ? 'IS NULL' : ' = '.$this->_conn->quote2($old_id_aclsbjgrp,false)).'';
    return $this->_conn->exec ($__query);
