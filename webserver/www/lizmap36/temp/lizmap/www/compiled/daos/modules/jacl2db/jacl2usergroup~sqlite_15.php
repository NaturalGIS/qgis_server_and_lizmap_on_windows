...
if (jApp::config()->compilation['checkCacheFiletime']&&(
 ...
 filemtime('C:\webserver\www\lizmap36\lizmap\vendor\jelix\jelix\lib\jelix-modules\jacl2db\daos/jacl2usergroup.dao.xml') > 1696258090)){ return false;
}
else {
 require_once ( JELIX_LIB_PATH .'dao/jDaoRecordBase.class.php');
 require_once ( JELIX_LIB_PATH .'dao/jDaoFactoryBase.class.php');

class cDaoRecord_jacl2db_Jx_jacl2usergroup_Jx_sqlite extends jDaoRecordBase {
 public $login;
 public $id_aclgrp;
   public function getSelector() { return "jacl2db~jacl2usergroup"; }
   public function getProperties() { return cDao_jacl2db_Jx_jacl2usergroup_Jx_sqlite::$_properties; }
   public function getPrimaryKeyNames() { return cDao_jacl2db_Jx_jacl2usergroup_Jx_sqlite::$_pkFields; }
}

class cDao_jacl2db_Jx_jacl2usergroup_Jx_sqlite extends jDaoFactoryBase {
  ...
  array (
   ...
   protected $_DaoRecordClassName='cDaoRecord_jacl2db_Jx_jacl2usergroup_Jx_sqlite';
...
public function update ($record){
     throw new jException('jelix~dao.error.update.impossible',array('jacl2db~jacl2usergroup','C:\webserver\www\lizmap36\lizmap\vendor\jelix\jelix\lib\jelix-modules\jacl2db\daos/jacl2usergroup.dao.xml'));
 }
...
$__query .=' WHERE  ug.id_aclgrp '.' = '.$this->_conn->quote($grp).' ORDER BY ug.login '.( strtolower($ordre) =='asc'?'asc':'desc').'';
 ...
 function getUsersGroupLimit ( $grp, $offset, $count, $orderDir='asc', $order='login'){
    $__query =  $this->_selectClause.$this->_fromClause.$this->_whereClause;
$__query .=' WHERE  ug.id_aclgrp '.' = '.$this->_conn->quote($grp).' ORDER BY '.$order.' '.( strtolower($orderDir) =='asc'?'asc':'desc').'';
 ...
 function getUsersGroupLimitAndFilter ( $grp, $login, $offset, $count, $orderDir='asc', $order='login'){
    $__query =  $this->_selectClause.$this->_fromClause.$this->_whereClause;
$__query .=' WHERE  ug.id_aclgrp '.' = '.$this->_conn->quote($grp).' AND ug.login '.' LIKE '.$this->_conn->quote($login).' ORDER BY '.$order.' '.( strtolower($orderDir) =='asc'?'asc':'desc').'';
    $__rs = $this->_conn->limitQuery($__query, $offset, $count);
    $this->finishInitResultSet($__rs);
    return $__rs;
}
